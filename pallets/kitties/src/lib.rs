...
pub mod pallet {
    ...
    /// Stores kitties prices. Key is kitty_id
    #[pallet::storage]
    #[pallet::getter(fn kitty_price)]
    pub type KittiesExchange<T: Config> =
    ...
    pub enum Event<T: Config> {
        ...
        /// A price for a kitty is set. \[kitty_id, new_price\]
        KittyPriceSet(T::KittyIndex, T::Balance),
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_price(
            ...
            // ensure ownership
                ...
                Self::kitties(&sender, kitty_id).is_some(),
                Error::<T>::InvalidKittyId
            );

            KittiesExchange::<T>::try_mutate(kitty_id, |current_price| -> DispatchResult {
                let price = Some(new_price);
                *current_price = price;
                Self::deposit_event(Event::KittyPriceSet(kitty_id, new_price));
                Ok(())
            })
        }

        /// Buy a kitty for the strike price
        #[pallet::weight(1000)]
        pub fn buy_kitty(
            origin: OriginFor<T>,
            owner: T::AccountId,
...
impl<T: Config> Pallet<T> {
    ...
    fn transfer_kitty(
        from: T::AccountId,
        to: T::AccountId,
        kitty_id: <T as pallet::Config>::KittyIndex,
    ) -> DispatchResult {
        Kitties::<T>::try_mutate_exists(from.clone(), kitty_id, |kitty| -> DispatchResult {
            if from == to {
