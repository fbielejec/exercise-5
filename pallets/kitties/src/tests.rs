...
fn can_transfer() {
    new_test_ext().execute_with(|| {
        ...
        // can't transfer non-existing kitty
        assert_noop!(
            KittiesModule::transfer(Origin::signed(100), 101, 1),
            Error::<Test>::InvalidKittyId
        );
        ...
        System::assert_last_event(Event::KittiesModule(
            crate::Event::<Test>::KittyTransferred(100u64, 101u64, 0u32),
        ));

        // can't transfer not your kitty
        assert_noop!(
            KittiesModule::transfer(Origin::signed(100), 101, 0),
            Error::<Test>::InvalidKittyId
        );
    });
}
